...
using tcomp1.Data;
using tcomp1.sendEmail;

var builder = WebApplication.CreateBuilder(args);
var connectionString = builder.Configuration.GetConnectionString("tcomp1ContextConnection");

builder.Services.AddDbContext<tcomp1Context>(options =>
    options.UseSqlServer(connectionString));

builder.Services.AddDefaultIdentity<tcomp1User>(options => options.SignIn.RequireConfirmedAccount = true)
    .AddRoles<IdentityRole>()
    .AddEntityFrameworkStores<tcomp1Context>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    string[] roleNames = { "User", "QA", "Admin" };
